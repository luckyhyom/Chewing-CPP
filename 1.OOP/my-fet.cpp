...
/*
    추후에 Bird, Fish같은 코드들이 필요해진다면 클래스가 필요해진다.
    1. Animal에 의해 만들어진 동물 관련 코드에 공통으로 쓰일 수 있는 필드와 함수를 재사용 하기 위해서
    2. 여러 동물을 하나의 자료형으로써 취급하기 위해서 (하나의 배열로써)
    3. 클래스에서 함수를 가지고 있으면 코드를 1인칭 관점에서 짤 수 있다. (나는 A한다.) 이건 그냥 스타일의 차이인듯
*/
